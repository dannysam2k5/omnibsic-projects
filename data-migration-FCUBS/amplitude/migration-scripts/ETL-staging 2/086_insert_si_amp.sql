create or replace procedure insert_si_amp as 
a number;
sys_date date;
...
'N'  apply_chg_suxs,         --5   APPLY_CHG_SUXS      --dan
trim(authorize)  authorize,         --6   AUTHORIZE
to_date(trim(book_date), 'DD-MM-RRRR')  book_date,         --7   BOOK_DATE
...
case  when trim(product_type)='O' then (select z.cust_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.dr_account) dr_account from sitb_upload_master@ampdblink y)) and z.alt_ac_no=trim(dr_account))
      when trim(product_type)='P' then (select z.cust_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.dr_account) dr_account from sitb_upload_master@ampdblink y)) and z.alt_ac_no=trim(dr_account))
      when trim(product_type)='I' then (select z.cust_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.cr_account) cr_account from sitb_upload_master@ampdblink y)) and z.alt_ac_no=trim(cr_account)) end counterparty,         --12   COUNTERPARTY
/*      
SIPC--PAYMENT/0NE TO MANY     (P)
SOCU--SWEEP-IN/0NE TO MANY    (S) 
    --SWEEP-OUT               (O)
    --COLLECTION              (C)
    --VARIABLE PAYMENT        (V)
    
*/
decode(trim(cr_account),(select z.alt_ac_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.cr_account) cr_account from sitb_upload_master@ampdblink y)) and z.alt_ac_no=trim(cr_account)),(select z.cust_ac_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.cr_account) cr_account from sitb_upload_master@ampdblink y)) and z.alt_ac_no=trim(cr_account)),'no_acct')  cr_account,         --13   CR_ACCOUNT
trim(cr_acc_br)  cr_acc_br,         --14   CR_ACC_BR
trim(cr_acc_ccy)  cr_acc_ccy,         --15   CR_ACC_CCY
trim(detail_ref)  detail_ref,         --16   DETAIL_REF
decode(trim(dr_account),(select z.alt_ac_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.dr_account) dr_account from sitb_upload_master@ampdblink y)) and z.alt_ac_no=trim(dr_account)),(select z.cust_ac_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.dr_account) dr_account from sitb_upload_master@ampdblink y)) and z.alt_ac_no=trim(dr_account)),'no_acct')  dr_account,         --17   DR_ACCOUNT
...
0 min_bal_after_sweep,--trim(min_bal_after_sweep)  min_bal_after_sweep,         --27   MIN_BAL_AFTER_SWEEP
0 min_sweep_amt,--trim(min_sweep_amt)  min_sweep_amt,         --28   MIN_SWEEP_AMT    dan
...
to_date(trim(si_expiry_date), 'DD-MM-RRRR')  si_expiry_date,         --38   SI_EXPIRY_DATE
...
from  sitb_upload_master@ampdblink
where 1=1
...
--select z.alt_ac_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.cr_account) cr_account from sitb_upload_master@ampdblink y));
--(select z.cust_ac_no from sttm_cust_account z where trim(z.alt_ac_no) in (select trim(dr_account) from sitb_upload_master@ampdblink) and trim(z.alt_ac_no)=trim(ui_cr_account))
...
to_date(trim(b.book_date), 'DD-MM-RRRR') book_date,--trim(b.book_date)  book_date,         --3   BOOK_DATE  --trim(b.to_date(trim(b.REVISION_DATE), 'DD-MM-RRRR')) 
...
to_date(trim(b.first_exec_date), 'DD-MM-RRRR') first_exec_date, -- first_exec_date,         --12   FIRST_EXEC_DATE
to_date(trim(b.first_value_date), 'DD-MM-RRRR') first_value_date, --first_value_date,         --13   FIRST_VALUE_DATE
...
null instruction_no,--trim(b.instruction_no)  instruction_no,         --18   INSTRUCTION_NO
trim(b.inst_status)  inst_status,         --19   INST_STATUS
trim(b.inst_version_no)  inst_version_no,         --20   INST_VERSION_NO
null latest_cycle_date,--trim(b.latest_cycle_date)  latest_cycle_date,         --21   LATEST_CYCLE_DATE
trim(b.latest_cycle_no)  latest_cycle_no,         --22   LATEST_CYCLE_NO
to_date(trim(b.latest_version_date), 'DD-MM-RRRR')  latest_version_date,         --23   LATEST_VERSION_DATE
trim(b.month_end_flag)  month_end_flag,         --24   MONTH_END_FLAG
to_date(trim(b.next_exec_date), 'DD-MM-RRRR')  next_exec_date,         --25   NEXT_EXEC_DATE
to_date(trim(b.next_value_date), 'DD-MM-RRRR')  next_value_date,         --26   NEXT_VALUE_DATE
'NEW' operation,--trim(b.operation)  operation,         --27   OPERATION
...
1 serial_no,--trim(b.serial_no)  serial_no,         --32   SERIAL_NO
...
from  sitb_upload_instruction@ampdblink b
where 1=1
...
/*
------------------------------------------------------------
execute insert_si_amp();
--update sitb_contract_master set ui_dr_account = dr_account, ui_cr_account=cr_account;
...
select * from ertb_msgs where err_code ='SI-DEF-007';
select * from ertb_msgs where err_code ='SI-VALS-005';   ---->
select * from ertb_msgs where err_code ='SI-RESP-004';--SI-DEF-002
select * from ertb_msgs where err_code ='SI-DEF-002';
select * from ertb_msgs where err_code ='SI-VALS-016';--SI-VALS-035   Action Code Should be Ignore for Sweep In, Sweep Out and Range Balancing Sweep Product Type
select * from ertb_msgs where err_code ='SI-VALS-035';--SI-VALS-016 
select * from ertb_msgs where err_code ='SI-CON033';--SI-CON040 --SI Expiry Date $1 cannot be before First Value Date $2
select * from ertb_msgs where err_code ='SI-DEF-005';
select * from ertb_msgs where err_code ='SI-CON040';
------------------------------------------------------------
...
-------
select * from sitb_upload_master@ampdblink where dr_acc_ccy||'~'||dr_account  not in (select ccy||'~'||trim(cust_ac_no) from sttm_cust_account);
select * from sitb_upload_master where cr_acc_ccy||'~'||cr_account  not in (select ccy||'~'||cust_ac_no from sttm_cust_account);
---------------------
-----------------------------------------------------------------------------------------------------------------------------------
select source_seq_no,source_ref, dr_acc_br,dr_account,cr_acc_br,cr_account from sitb_upload_master@ampdblink where source_seq_no in 
...
set counterparty = case  
when trim(product_type)='O' then (select z.cust_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.dr_account) dr_account from sitb_upload_master@ampdblink y)) and z.alt_ac_no=trim(dr_account)) 
when trim(product_type)='I' then (select z.cust_no from sttm_cust_account z where (trim(z.alt_ac_no) in (select trim(y.cr_account) cr_account from sitb_upload_master@ampdblink y)) and z.alt_ac_no=trim(cr_account)) end;
...
Begin
 FOR I IN (SELECT DISTINCT BRANCH_CODE FROM CSTB_EXT_CONTRACT_STAT WHERE FUNCTION_ID='SIDTRONL' AND IMPORT_STATUS='U')
   LOOP
     global.pr_init(I.BRANCH_CODE, 'MIG_USER03');--Change Branch Code here
...
/*
select a.SOURCE_REF,a.SI_EXPIRY_DATE,
b.FIRST_EXEC_DATE, b.NEXT_EXEC_DATE, b.FIRST_VALUE_DATE, b.NEXT_VALUE_DATE
from
sitb_upload_master a,
sitb_upload_instruction b
where
a.SOURCE_SEQ_NO = b.SOURCE_SEQ_NO
and to_date(a.SI_EXPIRY_DATE) < to_date(b.FIRST_EXEC_DATE)

to_date(

select * from sitb_upload_master;--PRODUCT_CODE
select * from sitb_upload_instruction;

SIPC
SOCU
select distinct PRODUCT_CODE,PRODUCT_TYPE from sitb_upload_master;

select distinct PRODUCT_CODE, PRODUCT_TYPE from sitbs_instruction;
select * from sitbs_contract_master;
...
SOCU--SWEEP-IN/0NE TO MANY    (S) 
    ...
    --VARIABLE PAYMENT        (V)
-----------------------------------    
select * from sitb_upload_master@to_dtn where trim(USER_REF_NUMERIC) in (select trim(user_inst_no) from sitbs_instruction);
select * from sitb_upload_master@to_dtn where trim(source_ref) in (select trim(user_inst_no) from sitbs_instruction);

select * from sitb_upload_instruction@to_dtn;


select distinct PRODUCT_TYPE from sitb_upload_master@to_dtn;
select distinct product_code, product_type from sitb_upload_instruction@to_dtn;
