...
'B' CAL_HOL_EXCP,--	Forward, Backward, Lapse (default=B)
...
'B' PROCESSING_TIME	,--End of day=default, Begining of day
...
From bkvrt acc
where acc.eta in ('VA','FO') and acc.dde>=sysdate -- and ctr!='9' ;
and mdev <> 0; -- dsam: remove SI with 0 transfer amount
...
'NEW' ACTION_CODE	,--default=NEW
(case when (vrt.sud is null or vrt.sud <sysdate) then 'N' else (case when sua>=sysdate then 'O' else 'N' end) end) SUSPEND_EXECUTION,--	default=N
NULL GOAL_REF_NO	,--default=NULL
NULL GROUP_CODE
From Sitb_Upload_Master Acc, bkvrt vrt 
where vrt.eta in ('VA','FO') and vrt.dde>=sysdate  and vrt.eve=acc.contract_ref_no;
...
Update Sitb_Upload_Master set bookdate='&DATE_PARAM' ;
commit;
