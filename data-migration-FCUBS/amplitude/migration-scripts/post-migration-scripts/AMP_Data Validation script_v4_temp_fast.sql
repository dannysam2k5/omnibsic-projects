...
(
select nvl(count(*),0) from sttm_upload_cust_personal@ampdblink WHERE ppt_exp_date < date_param)
);
commit;
...
(
select nvl(count(*),0) from sttm_upload_cust_corporate@ampdblink where incorp_date > date_param )
);
commit;
...
(
select nvl(count(*),0) from sttm_upload_cust_personal@ampdblink where date_of_birth < add_months(to_date(date_param,'DD-MM-RRRR'),(-18 * 12))  --- not a minor made a minor
...
(
select nvl(count(*),0) from sttm_upload_cust_personal@ampdblink where date_of_birth > add_months(to_date(date_param,'DD-MM-RRRR'),(-18 * 12)) --- a minor but not made a minor
and  maintenance_seq_no in (select a.maintenance_seq_no from sttm_upload_customer@ampdblink a where a.customer_type = 'I')
and minor <> 'Y'
)
); 
...
insert into amp_validation_tab values('CIF','Individual customer cannot have record in sttm_upload_cust_shareholder','sttm_upload_cust_shareholder',
(
...
(
select nvl(count(*),0) from sttb_upload_cust_account_casa@ampdblink where display_iban_in_advices is not null and source_code <> 'DTN_ACCT_UPD'
)
);
commit;
...
(select dr_gl||cr_gl||dr_ho_line||cr_ho_line||cr_cb_line||dr_cb_line from sttm_account_class_status where account_class = a.account_class and status = 'NORM')
)
);
commit;
...
insert into amp_validation_tab values ('ACCOUNT','AC_STAT_DR_OVD field should be N','sttb_upload_cust_account_casa',
(
select nvl(count(*),0) from sttb_upload_cust_account_casa@ampdblink where AC_STAT_DR_OVD <> 'N'
)
);
commit;
...
select nvl(count(*),0) from sttb_upload_cust_account_casa@ampdblink where account_type = 'N' and source_code <> 'DTN_ACCT_UPD'
)
);
commit;
...
select nvl(count(*),0) from sttb_upload_cust_account_casa@ampdblink where account_type is not NULL and account_type <> 'N'
)
);
commit;
...
-- SLOW PERFORMING QUERY THIS MUST BE RUN
/*
---------------------------------------------------
...
---------------------------------------------------
-- 96. Payment_method should be defaulted to B
---------------------------------------------------
...
---------------------------------------------------
-- 97. Rollover_allowed should be defaulted to N
---------------------------------------------------
...
---------------------------------------------------
-- 98. Partial_payment_mliq should be defaulted to N
---------------------------------------------------
